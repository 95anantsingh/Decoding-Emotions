...
from models import GE2E, Dense, ProbingModel, ProbingDense, TransformerBridge
...
FX_MODELS = ['WAV2VEC2_BASE','WAV2VEC2_LARGE',
            ...
            'HUBERT_ASR_LARGE', 'WAV2VEC2_ASR_LARGE_960H']
...
FX_MODEL_MAP = {'WAV2VEC2_BASE':'WAV2VEC2_BASE','WAV2VEC2_LARGE':'WAV2VEC2_LARGE',
                ...
                'HUBERT_ASR_LARGE':'HUBERT_ASR_LARGE', 'WAV2VEC2_ASR_LARGE_960H':'WAV2VEC2_ASR_LARGE_960H'}
...
class Trainer:
    ...
    def __init__(self, config):
        ...
        if self.config.finetune_layer != None: self.ft_model = TransformerBridge(self.fx_model,self.clf_model,self.config.finetune_layer)
    ...
    def _config_check(self):
        # TODO check for finetuning possiblity with given fx model
        # TODO check finetune layer if finetuning is possible layer less than 12 or 24
        # TODO check for possible extract modes implimented with a fx and clf
        pass
    ...
    def _gpu_finetune(self, model, dataloader, optimizer, criterion, progress_bar):
        total_loss = 0.0
        model.train()
        for batch in dataloader:
            batch = tuple(input.to(self.device) for input in batch)
    ...
    def run_pipeline(self):
        ...
        with logging_redirect_tqdm(loggers=[self.logger, self.no_fmt_logger]):
            ...
            if self.config.finetune_layer != None: 
                

            elif self.config.clf_model=='PROBING' or self.config.clf_model=='PROBING_DENSE': #and self.config.fx_model!='GE2E':
                models, optimizers = [], []
